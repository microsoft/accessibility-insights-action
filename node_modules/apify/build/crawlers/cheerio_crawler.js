...
class CheerioCrawler {
     ...
     */
    constructor(options = {}) {
        const { requestOptions, handlePageFunction, requestTimeoutSecs = 30, handlePageTimeoutSecs = 60, ignoreSslErrors = true, additionalMimeTypes = [], suggestResponseEncoding, forceResponseEncoding, proxyConfiguration, 
        ...
        utilities_1.checkParamPrototypeOrThrow(proxyConfiguration, 'options.proxyConfiguration', proxy_configuration_1.ProxyConfiguration, 'ProxyConfiguration', true);
    ...
    async _handleRequestFunction({ request, autoscaledPool, session }) {
        ...
        if (this.prepareRequestFunction)
            await this.prepareRequestFunction({ request, session, proxyInfo });
        const { dom, isXml, body, contentType, response } = await utils_2.addTimeoutToPromise(this._requestFunction({ request, session, proxyUrl }), this.requestTimeoutMillis, `request timed out after ${this.requestTimeoutMillis / 1000} seconds.`);

...
class ProxyConfiguration {
    ...
    constructor(options = {}) {
        ...
        utils_1.checkParamOrThrow(newUrlFunction, 'options.newUrlFunction', 'Maybe Function');
        ...
        if (proxyUrls)
            this._validateProxyUrls(proxyUrls);
        if (countryCodeToUse)
            this._validateCountryCode(countryCodeToUse);
    ...
    _validateSessionArgumentStructure(sessionId) {
        if (!regexs_1.APIFY_PROXY_VALUE_REGEX.test(sessionId))
            this._throwInvalidProxyValueError('session', sessionId);
    ...
    _validateGroupsStructure(groups) {
        for (const group of groups) {
            if (!regexs_1.APIFY_PROXY_VALUE_REGEX.test(group))
                this._throwInvalidProxyValueError('group', group);
        }
    }
    /**
